...
import { KubeDeployment, KubeRole, KubeRoleBinding, KubeSecret, KubeServiceAccount } from './imports/k8s';
...
export class MongoDbChart extends Chart {
  constructor(scope: Construct, id: string, props: ChartProps = { }) {
    ...
    // psmdb operator
    const psmdb_operator_role = new KubeRole(this, 'psmdb-operator-role', {
      metadata: {
    ...
    const psmdb_operator_sa = new KubeServiceAccount(this, 'psmdb-operator-sa', {
      metadata: {
        name: 'percona-server-mongodb-operator'
      }
    });

    new KubeRoleBinding(this, 'psmdb-operator-rolebinding', {
      metadata: {
    ...
    new KubeDeployment(this, 'psmdb-operator-deployment', {
      metadata: {
        name: 'percona-server-mongodb-operator'
      }, 
      spec: {
    ...
    new KubeSecret(this, 'psmdb-secrets', {
      metadata: {
        name: `${id}-secrets`
      },
      type: 'Opaque',
...
const app = new App();
new MongoDbChart(app, 'doik-cdk8s');
app.synth();
